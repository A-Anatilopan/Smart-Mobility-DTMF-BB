...
type MappaServizioLeafletComponent = ComponentType<MappaServizioProps>;

function MappaServizioLoadingFallback() {
  return (
...
// progetto, ma carica Leaflet solo dopo il mount del browser. Cosi evitiamo
// il bailout HTML lasciato da `ssr: false` nella home amministrazione.
export default function MappaServizioMock(props: MappaServizioProps) {
  const [MappaServizioLeaflet, setMappaServizioLeaflet] =
    useState<MappaServizioLeafletComponent | null>(null);

  useEffect(() => {
    let attivo = true;

    void import("@/components/mappa/MappaServizioLeaflet").then((modulo) => {
      if (!attivo) {
        return;
      }

      setMappaServizioLeaflet(() => modulo.default);
    });
  ...
  if (!MappaServizioLeaflet) {
    return <MappaServizioLoadingFallback />;
  }
